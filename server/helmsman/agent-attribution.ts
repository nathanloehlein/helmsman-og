...
const LEGACY_BYLINE = new RegExp(`^_Helmsman (?:review agent|PR author) · model: ${MODEL_FIELD} · effort: ${EFFORT_FIELD}_$`, 'i');
const BYLINE = new RegExp(`^_Helmsman · ${MODEL_FIELD} - (?:min|low|med|high|xhigh|max|not reported)_$`, 'i');
...
function attributionField(value: string | undefined | null, kind: 'model' | 'effort'): string {
  if (value === PROVIDER_DEFAULT || value === NOT_REPORTED) return NOT_REPORTED;
  return kind === 'model'
...
export function stripAgentByline(body: string): string {
  ...
  while (content) {
    ...
    if ((!BYLINE.test(lastLine) && !LEGACY_BYLINE.test(lastLine)) || preceding && (!/\n\s*$/.test(preceding) || unclosedFence(preceding))) break;
...
export function appendAgentByline(body: string, attribution: AgentAttribution): string {
  ...
  if (fence) content += `\n${fence}`;
  const effort = attributionField(attribution?.effort, 'effort');
  const effortLabel = effort === 'medium' ? 'med' : effort === 'minimal' ? 'min' : effort;
  const footer = `_Helmsman · ${attributionField(attribution?.model, 'model')} - ${effortLabel}_`;
