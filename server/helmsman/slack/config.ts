...
export const SLACK_CONFIG_KEYS = ['SLACK_ENABLED', 'SLACK_WATCH_ENABLED', 'SLACK_CLIENT_ID', 'SLACK_CHANNEL_ID', 'SLACK_CHANNEL_NAME', 'SLACK_BROWSER_SURFACE', 'SLACK_BROWSER', 'SLACK_FIREFOX_WEBDRIVER_URL',
  'SLACK_TRANSPORT', 'SLACK_MCP_TEAM_ID', 'SLACK_REVIEW_GROUP_ID', 'SLACK_OAUTH_CLIENT_ID', 'SLACK_OAUTH_REDIRECT_URI'] as const;
...
export function slackSettings(env: Record<string, string | undefined>) {
  ...
  const transportValue = env.SLACK_TRANSPORT?.trim() || 'browser';
  const transport: 'browser' | 'mcp' = transportValue === 'mcp' ? 'mcp' : 'browser';
  const clientId = transport === 'mcp' ? slackMcpTeamId(env) : env.SLACK_CLIENT_ID?.trim() ?? '';
  ...
  let error: string | null = ['browser', 'mcp'].includes(transportValue) ? null : 'Slack transport must be browser or mcp.';
  if (transport === 'browser' && browserValue !== 'cmux' && browserValue !== 'firefox') error ??= 'Slack browser must be cmux or firefox.';
  ...
  catch (cause) { if (transport === 'browser') error ??= cause instanceof Error ? cause.message : 'Firefox WebDriver configuration is invalid.'; }
  ...
  if (transport === 'browser' && surface !== undefined && !(browser === 'firefox' ? firefoxSurface(surface) : cmuxSurface)) {
  ...
  return { enabled, clientId, channelId, channelName, surface, browser, firefoxWebDriverUrl, transport, error };
}

export function slackMcpTeamId(env: Record<string, string | undefined>): string {
  return env.SLACK_MCP_TEAM_ID?.trim() || (/^T[A-Z0-9]{2,31}$/.test(env.SLACK_CLIENT_ID ?? '') ? env.SLACK_CLIENT_ID! : '');
}
...
export function publicSlackSettings(env: Record<string, string | undefined>): Record<string, unknown> {
  ...
  return {
    ...
    SLACK_CLIENT_ID: env.SLACK_CLIENT_ID?.trim() ?? '',
    ...
    SLACK_TRANSPORT: settings.transport,
    SLACK_MCP_TEAM_ID: slackMcpTeamId(env),
    SLACK_REVIEW_GROUP_ID: env.SLACK_REVIEW_GROUP_ID?.trim() ?? '',
    SLACK_OAUTH_CLIENT_ID: env.SLACK_OAUTH_CLIENT_ID?.trim() ?? '',
    SLACK_OAUTH_REDIRECT_URI: env.SLACK_OAUTH_REDIRECT_URI?.trim() ?? '',
  };
}
