...
import type { OutcomeRun, OutcomeSummary, UsageCostEstimate } from './data/outcomes';
...
function estimateLabel(estimate: UsageCostEstimate | undefined): string {
  if (!estimate) return 'Unknown';
  if (estimate.estimatedUnreportedCostUsd === null) return `Unknown · 0/${estimate.unreportedUsageEvents} unpriced records`;
  const amount = estimate.estimatedUnreportedCostUsd;
  return `$${amount.toFixed(amount > 0 && amount < 0.01 ? 4 : 2)} estimated · ${estimate.estimatedUsageEvents}/${estimate.unreportedUsageEvents} unpriced records`;
}
...
export function renderOutcomesView(state: OutcomesViewState, opts: HelmHeadOpts): string {
  ...
  const trendRows = summary.daily.map(day => `<tr><th scope="row">${esc(day.date)}</th><td>${day.runs}</td><td>${day.succeeded}</td><td>${day.failed}</td><td>${money(day.observedCostUsd)}</td><td>${esc(estimateLabel(day.costEstimate))}</td><td>${day.runsWithKnownCost}/${day.runs}</td></tr>`).join('') || '<tr><td colspan="7">No daily data.</td></tr>';
  const runs = summary.recentRuns.map(run => `<li class="outcome-run"><a class="app-link" href="${esc(routeHref({ view: 'runs', repo: opts.selectedRepo, run: run.runId }))}">${esc(run.runId)}</a><span>Execution: ${esc(run.status)}</span><span>${run.costUsd === null && run.observedCostUsd !== null ? `${money(run.observedCostUsd)} reported · total unknown` : money(run.costUsd)}</span>${run.costEstimate?.unreportedUsageEvents ? `<span>Unreported usage: ${esc(estimateLabel(run.costEstimate))}</span>` : ''}<span>${duration(run.durationMs)}</span>${assessmentForm(run, state.savingRunId === run.runId)}</li>`).join('') || `<li class="empty-note">No ${term('runs').toLowerCase()} started in this time period for the selected scope. Choose a longer period or another ${term('repository').toLowerCase()}.</li>`;
  return renderAppShell({ ...opts, active: 'outcomes' }, `<section class="outcomes-view">
    ...
    <section class="panel"><div class="panel-head"><span class="panel-title">Standard API estimate</span></div><div class="outcome-stats">${stat('Unreported usage estimate', estimateLabel(summary.costEstimate), `${summary.costEstimate?.runsWithoutUsage ?? summary.runs} ${(summary.costEstimate?.runsWithoutUsage ?? summary.runs) === 1 ? 'run' : 'runs'} without usage records`)}</div><p class="empty-note">Estimates cover only unpriced Codex usage with a known model and complete input, cached-input and output counts. Rates assume Standard short-context API pricing; service tier, long-context requests and cache-write usage are not recorded. Actual charges can differ, including on subscription plans. Estimates do not affect reported spend, per-PR costs or budget caps. Unknown records and missing usage remain excluded.</p>${summary.costEstimate ? `<p class="empty-note"><a href="https://developers.openai.com/api/docs/pricing" target="_blank" rel="noreferrer">OpenAI API rates</a> checked ${esc(summary.costEstimate.ratesAsOf)}. Coverage counts recorded usage events, not complete billing.</p>` : ''}</section>
