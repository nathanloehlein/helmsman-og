...
function renderItem(item: Todo, state: TodosViewState): string {
  ...
  const stateLabel = STATE_LABELS[item.state] ?? 'Unknown';
  return `<li class="todo-item${state.editingId === item.id ? ' is-editing' : ''}" data-todo-id="${esc(item.id)}">
    ...
    ${item.description ? `<p class="todo-description">${esc(text(item.description))}</p>` : '<p class="todo-missing">Draft · add a description to make this todo ready for a voyage.</p>'}
