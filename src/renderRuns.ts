  ...
  return renderAppShell({ active: 'runs', repos, selectedRepo: opts.selectedRepo, themeId: opts.themeId, readout: { running: runs.filter(run => run.status === 'running').length, queued: null, review: null } }, `
    <section class="panel pr-lookup-panel" data-pane="newrun">
      <div class="panel-head">
        ...
        <a class="app-link runs-pane-link" href="${esc(paneHref('newrun', opts.selectedRepo, state.number, state.repo))}" aria-label="Link to Run a PR">Link ↗</a>
      </div>
      <div class="pr-lookup-form">
        <input class="pr-lookup-input" aria-label="PR to run" placeholder="Paste a PR URL or owner/name#number" value="${esc(value)}" />
    ...
    <section class="panel runs-recent-panel" data-pane="recent">
      ...
      <ul class="recent-runs-list lane-list">${runs.length ? runs.map(run => renderVoyage(run, undefined, opts.selectedRepo)).join('') : '<li class="empty-note">No voyages yet.</li>'}</ul>
    </section>
