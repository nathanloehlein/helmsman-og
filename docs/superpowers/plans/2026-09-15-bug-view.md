...
**Tech Stack:** TypeScript (strict, `--erasableSyntaxOnly` — NO constructor parameter properties), Node `node:http` orchestrator run under `tsx`, Vite front end (pure logic in `src/logic/`, DOM in `src/render.ts`/`src/main.ts`), Vitest + jsdom.
...
**Files:**
- Modify: `server/orchestrator/router.ts` (add `bugs` dep + route)
- Modify: `server/orchestrator/main.ts` (provide the dep)
- Test: `server/orchestrator/router.test.ts` (add a route test)
...
Append to `server/orchestrator/router.test.ts` (match the file's existing `handleApi` call style and deps fixture; add a `bugs` entry to whatever deps builder it uses). Minimal test:
...
- [ ] **Step 3: Add the dep + route in `server/orchestrator/router.ts`**
...
- [ ] **Step 4: Provide the dep in `server/orchestrator/main.ts`**
...
Run: `npx vitest run server/orchestrator/router.test.ts && npx tsc --noEmit`
